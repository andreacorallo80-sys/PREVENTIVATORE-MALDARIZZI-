...
# Variabili per gestire le gomme senza mandare in crash l'app
...
if check_password():
            ...
            try:
                ...
                with c_player:
                    if 'PLAYER' in df_promo.columns:
                        lista_player = ["Tutte"] + sorted([str(x).upper() for x in df_promo['PLAYER'].unique() if str(x).strip()])
                ...
                for _, row in df_promo.iterrows():
                    ...
                    if filtro_player != "Tutte" and player != filtro_player: continue
                ...
                # --- ORDINAMENTO OFFERTE DAL CANONE PIU BASSO ---
                ...
                if not offerte_filtrate:
                    ...
                    for idx, auto in enumerate(offerte_filtrate):
                        with colonne_griglia[idx % 3]:
                            ...
                            if st.button(f"➡️ Utilizza la Promo", key=f"btn_promo_{idx}"):
                                ...
                                # --- CALCOLO KM ANNUALI (Totali / Mesi * 12) ---
                                dur = auto['durata']
    ...
    # ==========================================
        ...
        with s3:
            # --- AGGIUNTA STEP: OPZIONE GOMME BLINDATA ---
